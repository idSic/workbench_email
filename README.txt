...
transitions. Based on those email transitions, the admin can configure the
each email's subject / message. Then when the content moves through the
specific transition, if an email transition is already set, the content creator
has the ability to send email to those specific user(s).
...
This is my first drupal contributed module, so I know there are room for
...
I know of (rules, actions, triggers etc but no easily configurable system). So,
I'm curious to see what other people think about this, feel free to comment :)

----
6.1  Database schema

Workbench Email uses two tables to track emails and email transitions.
